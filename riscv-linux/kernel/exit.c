...
/* cleanup when traced task exits */
static void hwtrace_exit(struct task_struct *t) {
  printk("[EXIT] RISCV trace end\n");
	printk("\t|-- name = %s, pid = %d\n"
		"\t|-- cmap = %px, sz = %u\n"
    "\t|-- ht_saddr = %px, ht_eaddr = %px\n"
		"\t|-- lbq = %px, sz = %u\n",
			t->comm, t->pid,
			t->cmap, t->msz,
      t->ht_saddr, t->ht_eaddr,
			t->lbq, t->qsz
	);
	
  /* read lbq and disable trace */
	__ht_disable();
  __flush_lbq(t);
...
void __noreturn do_exit(long code)
{
	...
	/* Deliver task info with cmap if available */
	if (tsk->cmap)
		hwtrace_exit(tsk);
