...
extern unsigned int umap;
extern void *HT_CMAP[HT_CNT];
...
static int ht_open(struct inode *inodep, struct file *filep) {
	...
	if (tmap == HT_CNT) {
		printk(KERN_ALERT "[CMAP_ERR] Exceeding allowance!\n");
		return -EFAULT;
	}

	i = tmap;
	if ((HT_CMAP[i] = kmalloc(HT_MSZ, GFP_KERNEL)) == NULL) {
		printk(KERN_ALERT "[CMAP_ERR] No memory!\n");
		return -ENOMEM;
	}
	// set reserved bit for not swapping out
	for (j = 0; j < HT_MSZ; j += PAGE_SIZE)
		SetPageReserved(virt_to_page(((unsigned long)HT_CMAP[i]) + j));

	printk(KERN_ALERT "[CMAP] Open CMAP #%u (%px)\n", i, HT_CMAP[i]);
	tmap++;
	return 0;
}
...
static int ht_mmap(struct file *filep, struct vm_area_struct *vma) {
	...
	if (umap == tmap) {
		printk(KERN_ALERT "[CMAP_ERR] Exhausted, open() before mmap()!\n");
		return -EFAULT;
	}
	if (size != HT_MSZ) {
