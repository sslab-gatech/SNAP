   ...
   with HasBoomUOP
{
  ...
  val state               = UInt(2.W)
  val prev_used           = Bits(coreMaxAddrBits.W)
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    val exe_br_val = brq(i).valid && !IsKilledByBranch(io.core.brinfo, brq(i).bits.uop) &&
                      io.core.brinfo.ht_valid && i.U === io.core.brinfo.brq_idx
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      when (IsKilledByBranch(io.core.brinfo, brq(i).bits.uop)) {
        ...
        printf("YH+ [%d] BRQ S_INIT, brq_idx: %d brq_head: %d brq_tail: %d target: %x inst_hash: %x prev_used: %x prev_new: %x\n",
                io.core.tsc_reg, i.U, brq_head, brq_tail, io.core.brinfo.target,
                brq(i).bits.uop.inst_hash, temp_prev_hash,
                ((io.core.brinfo.target ^ brq(i).bits.uop.inst_hash) >> 1))
        ...
        brq(i).bits.prev_used           := temp_prev_hash
  ...
  when (reset.asBool || initBRQ) {
    ...
    when (brq(bidx).valid && !IsKilledByBranch(io.core.brinfo, brq(bidx).bits.uop)) {
      ...
      //prev_hash := brq(bidx).bits.prev_new
      //prev_hash := ((io.core.brinfo.target ^ brq(bidx).bits.uop.inst_hash) >> 1)
    } .otherwise {
      printf("YH+ [%d] Killed by branch, uopc: %d\n",
              io.core.tsc_reg, brq(bidx).bits.uop.uopc)

      brq_tail := io.core.brinfo.brq_idx
      //prev_hash := brq(bidx).bits.prev_used
    }
