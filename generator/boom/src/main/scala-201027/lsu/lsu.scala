   ...
   with HasBoomUOP
{
  ...
  val state               = UInt(2.W)
  val prev_new            = Bits(coreMaxAddrBits.W)
  val forwarded           = Bool()
}
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  //io.core.brqNonEmpty := brq_nonempty
  //io.core.brqHead := brq_head
  //io.core.brqTail := brq_tail
  //io.core.brqCommitHead := brq_commit_head
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (commit_br) {
      printf("YH+ [%d] BRQ COMMIT, brq_idx: %d pc: %x addr: %x prev_used: %x prev_new: %x\n",
              io.core.tsc_reg, bidx, brq(bidx).bits.addr.bits,
              (bitmap_base | ((brq(bidx).bits.addr.bits ^ brq(bidx).bits.uop.inst_hash ^
                                        temp_prev_hash) & bitmap_size)),
              temp_prev_hash,
              ((brq(bidx).bits.addr.bits ^ brq(bidx).bits.uop.inst_hash) >> 1))

      brq(bidx).bits.addr.bits := bitmap_base |
                                    ((brq(bidx).bits.addr.bits ^ brq(bidx).bits.uop.inst_hash ^
                                      temp_prev_hash) & bitmap_size)
      brq(bidx).bits.prev_new  := ((brq(bidx).bits.addr.bits ^ brq(bidx).bits.uop.inst_hash) >> 1)
    ...
    temp_prev_hash = Mux(commit_br, ((brq(bidx).bits.addr.bits ^ brq(bidx).bits.uop.inst_hash) >> 1),
                                    temp_prev_hash)
  }
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      //  printf("YH+ [%d] BRQ S_INIT, brq_idx: %d brq_head: %d brq_tail: %d target: %x inst_hash: %x\n",
      //          io.core.tsc_reg, i.U, brq_head, brq_tail, io.core.brinfo.target,
      //          brq(i).bits.uop.inst_hash)

      //  brq(i).bits.addr.valid          := true.B
      //  brq(i).bits.addr.bits           := io.core.brinfo.target
      //  brq(i).bits.state               := s_load
      //  brq(i).bits.uop.uopc            := uopLD
      //  brq(i).bits.uop.mem_size        := 0.U
      //  brq(i).bits.uop.is_htload       := true.B
