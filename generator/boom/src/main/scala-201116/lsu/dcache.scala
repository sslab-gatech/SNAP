  ...
  with HasBoomCoreParameters
{
  ...
  //val total_miss_cnt = RegInit(0.U(xLen.W))
  //val ht_miss_cnt = RegInit(0.U(xLen.W))

  //var temp_total_miss_cnt = total_miss_cnt
  //var temp_ht_miss_cnt = ht_miss_cnt

  //for (w <- 0 until memWidth) {
  //  val is_total_miss = s2_valid(w)          &&
  //                      !s2_hit(w)            &&
  //                      !s2_nack_hit(w)       &&
  //                      !s2_nack_victim(w)    &&
  //                      !s2_nack_data(w)      &&
  ...
  //  temp_total_miss_cnt = Mux(io.lsu.enableDebug && is_total_miss, temp_total_miss_cnt + 1.U,
  //                            temp_total_miss_cnt)

  //  temp_ht_miss_cnt = Mux(io.lsu.enableDebug && is_ht_miss, temp_ht_miss_cnt + 1.U,
  //                            temp_ht_miss_cnt)

  //  when (io.lsu.enableDebug && is_total_miss) {
  //    printf("YH+ total_miss_cnt: %d\n", temp_total_miss_cnt)
  //  }

  //  when (io.lsu.enableDebug && is_ht_miss) {
  //    printf("YH+ ht_miss_cnt: %d\n", temp_ht_miss_cnt)
  //  }
  //}

  //when (initDebug) {
  //  printf("YH+ Reset miss_cnt total: %d ht: %d\n", io.lsu.regTotalMissCnt, io.lsu.regHtMissCnt)

  //  total_miss_cnt := io.lsu.regTotalMissCnt
  //  ht_miss_cnt := io.lsu.regHtMissCnt
  //} .elsewhen(regEnableDebug) {
  //  total_miss_cnt := temp_total_miss_cnt
  //  ht_miss_cnt := temp_ht_miss_cnt
  //}

  //io.lsu.totalMissCnt := total_miss_cnt
  //io.lsu.htMissCnt := ht_miss_cnt
  //yh+end
