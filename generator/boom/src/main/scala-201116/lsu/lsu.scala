...
class LSUHtConfig(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val enableBRQ     = Bool()
  val enableLBQ     = Bool()
  //val enableDebug   = Bool()
  val flushLBQ      = Bool()
  val prev	        = UInt(coreMaxAddrBits.W)
  val brqFwdCnt     = UInt(xLen.W)
  val brqCommitCnt  = UInt(xLen.W)
  //val totalMissCnt  = UInt(xLen.W)
  //val htMissCnt     = UInt(xLen.W)
}
//yh+end
...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  //yh+begin
  //val enableDebug = Output(Bool())
  //val regTotalMissCnt = Output(UInt(xLen.W))
  //val regHtMissCnt = Output(UInt(xLen.W))
  //val totalMissCnt = Input(UInt(xLen.W))
  //val htMissCnt = Input(UInt(xLen.W))
  //yh+end
}
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  //val totalMissCnt = Output(UInt(xLen.W))
  //val htMissCnt = Output(UInt(xLen.W))
  //yh+end
   ...
   with HasBoomUOP
{
  ...
  val state               = UInt(2.W)
  val hash_new            = Bits(coreMaxAddrBits.W)
  val hash_prev           = Bits(coreMaxAddrBits.W)
  val forwarded           = Bool()
}
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val regPrevHash      = Reg(UInt(xLen.W))

  //val regTotalMissCnt  = Reg(UInt(xLen.W))
  //val regHtMissCnt     = Reg(UInt(xLen.W))
  ...
  //regTotalMissCnt := io.core.ht_config.totalMissCnt
  //regHtMissCnt := io.core.ht_config.htMissCnt
  //io.dmem.enableDebug := io.core.ht_config.enableDebug
  //io.dmem.regTotalMissCnt := regTotalMissCnt
  //io.dmem.regHtMissCnt := regHtMissCnt
  //io.core.totalMissCnt := io.dmem.totalMissCnt
  //io.core.htMissCnt := io.dmem.htMissCnt
  ...
  val brq_store_idx     = RegInit(0.U(brqAddrSz.W)) // point to next store to execute
  ...
  // Dispatch
  val prev_hash         = Reg(UInt(coreMaxAddrBits.W))
  ...
  for (w <- 0 until coreWidth) {
    ...
    val dis_br_val = regEnableBRQ && io.core.dis_uops(w).valid &&
                      io.core.dis_uops(w).bits.uses_brq && !io.core.dis_uops(w).bits.exception

    when (dis_br_val) {
      brq(br_enq_idx).valid               := true.B
      brq(br_enq_idx).bits.addr.valid     := true.B
      //brq(br_enq_idx).bits.addr.bits      := io.core.dis_uops(w).bits.debug_pc
      brq(br_enq_idx).bits.addr.bits      := bitmap_base |
                                              ((io.core.dis_uops(w).bits.debug_pc ^ io.core.dis_uops(w).bits.inst_hash ^
                                                temp_prev_hash) & bitmap_size)
      brq(br_enq_idx).bits.hash_new       := ((io.core.dis_uops(w).bits.debug_pc ^ io.core.dis_uops(w).bits.inst_hash) >> 1)
      brq(br_enq_idx).bits.hash_prev      := temp_prev_hash
      brq(br_enq_idx).bits.state          := s_load  // state
      brq(br_enq_idx).bits.uop            := io.core.dis_uops(w).bits
      brq(br_enq_idx).bits.uop.uopc       := uopLD
      brq(br_enq_idx).bits.uop.mem_size   := 0.U
      brq(br_enq_idx).bits.uop.mem_cmd    := rocket.M_XRD
      brq(br_enq_idx).bits.uop.is_htload  := true.B
      brq(br_enq_idx).bits.uop.uses_ldq   := false.B
      brq(br_enq_idx).bits.uop.uses_stq   := false.B
      ...
      printf("YH+ [%d] Enqueue BRQ Entry, br_enq_idx: %d brq_head: %d brq_tail: %d uopc: %d debug_pc: %x inst_hash: %x\n",
    ...
    temp_prev_hash = Mux(dis_br_val, ((io.core.dis_uops(w).bits.debug_pc ^ io.core.dis_uops(w).bits.inst_hash) >> 1),
                                    temp_prev_hash)
  }
  ...
  for (w <- 0 until coreWidth) {
    val commit_br = io.core.commit.valids(w) && io.core.commit.uops(w).uses_brq

    var bidx = io.core.commit.uops(w).brq_idx

    when (commit_br) {
      printf("YH+ [%d] BRQ COMMIT, brq_idx: %d addr: %x\n",
              io.core.tsc_reg, bidx, brq(bidx).bits.addr.bits)

      brq(bidx).bits.committed := true.B
    }
  ...
  for (i <- 0 until numBrqEntries) {
    ...
    val commit_match = brq(i).bits.addr.valid && brq(i).bits.committed &&
                        IsOlder(brq_store_idx, i.U, brq_head) &&
                        brq(i).bits.addr.bits === brq(brq_store_idx).bits.addr.bits
                       
    match_mask(i) := 0.U

    //when (i.U === brq_head || commit_match) {
    when (i.U === brq_store_idx || commit_match) {
      match_mask(i) := 1.U
  ...
  for (i <- 0 until numBrqEntries)
  {
    printf("YH+ [%d] brq(%d) valid: %d addr: %x state: %d executed: %d committed: %d\n",
            io.core.tsc_reg, i.U, brq(i).valid, brq(i).bits.addr.bits, brq(i).bits.state, brq(i).bits.executed, brq(i).bits.committed)

    //val exe_br_val = (io.core.brinfo.ht_valid && i.U === io.core.brinfo.brq_idx)
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      when (IsKilledByBranch(io.core.brinfo, brq(i).bits.uop)) {
        ...
        assert(!brq(i).bits.committed)
      } .elsewhen (brq(i).bits.state === s_load || brq(i).bits.state === s_store) {
        //val forwarded = brq(i).bits.committed && i.U =/= brq_head &&
        //                brq(i).bits.addr.bits === brq(brq_head).bits.addr.bits &&
        //                (brq(brq_head).bits.state === s_load ||
        //                  (brq(brq_head).bits.state === s_store && !brq(brq_head).bits.executed))

        val forwarded = brq(i).bits.committed && i.U =/= brq_store_idx &&
                        IsOlder(brq_store_idx, i.U, brq_head) &&
                        brq(i).bits.addr.bits === brq(brq_store_idx).bits.addr.bits &&
                        (brq(brq_store_idx).bits.state === s_load ||
                          (brq(brq_store_idx).bits.state === s_store && !brq(brq_store_idx).bits.executed))

        when (forwarded) {
          assert(brq(i).bits.state =/= s_store)

          printf("YH+ [%d] BRQ S_FWD, brq_idx: %d brq_head: %d brq_store_idx: %d brq_tail: %d\n",
                  io.core.tsc_reg, i.U, brq_head, brq_store_idx, brq_tail)
      ...
      } .elsewhen (i.U === brq_head && brq(i).bits.state === s_done && brq(i).bits.committed) {
  ...
  val clear_br = brq(brq_head).valid && brq(brq_head).bits.state === s_done && !io.core.exception && brq(brq_head).bits.committed
  ...
  when (reset.asBool || initBRQ) {
    ...
    prev_hash := brq(brq_commit_head).bits.hash_new
    ...
    when (brq(bidx).valid) {
      ...
      when (!IsKilledByBranch(io.core.brinfo, brq(bidx).bits.uop)) {
        ...
        prev_hash := brq(bidx).bits.hash_new

        assert(!brq(bidx).bits.committed)
      } .otherwise {
        printf("YH+ [%d] Killed by branch, uopc: %d\n",
                io.core.tsc_reg, brq(bidx).bits.uop.uopc)

        brq_tail := io.core.brinfo.brq_idx
        prev_hash := brq(bidx).bits.hash_prev
  ...
  when (brq(brq_store_idx).valid && brq(brq_store_idx).bits.state === s_done) {
    //assert(brq(brq_store_idx).bits.forwarded)
    brq_store_idx := WrapInc(brq_store_idx, numBrqEntries)
  }
  //yh+end *** BRQ ***
  ...
  val lbq_commit_head   = RegInit(0.U(lbqAddrSz.W))

  var lb_enq_idx        = lbq_tail
  var temp_lbq_head     = lbq_head

  // Commit
  var temp_lbq_commit_head = lbq_commit_head

  for (w <- 0 until coreWidth) {
    //val commit_br = regEnableLBQ && io.core.commit.valids(w) &&
    //                  io.core.commit.uops(w).uses_brq && io.core.commit.uops(w).is_br_or_jmp

    val commit_br = regEnableLBQ && io.core.commit.valids(w) &&
                      io.core.commit.uops(w).uses_lbq

    var lbq_idx = io.core.commit.uops(w).lbq_idx

    when (commit_br) {
      lbq(lbq_idx).bits.committed := true.B
    }

    temp_lbq_commit_head = Mux(commit_br, WrapInc(temp_lbq_commit_head, numLbqEntries),
                               temp_lbq_commit_head)
  }
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    val dis_lb_val = regEnableLBQ && io.core.dis_uops(w).valid &&
                      io.core.dis_uops(w).bits.uses_lbq && !io.core.dis_uops(w).bits.exception

    when (dis_lb_val)
    {
      lbq(lb_enq_idx).valid             := true.B
      lbq(lb_enq_idx).bits.uop          := io.core.dis_uops(w).bits
      lbq(lb_enq_idx).bits.committed    := false.B

      printf("YH+ [%d] Enqueue LBQ Entry, lb_enq_idx: %d\n", io.core.tsc_reg, lb_enq_idx)

      assert(io.core.dis_uops(w).bits.is_br_or_jmp)
      assert(io.core.dis_uops(w).bits.uses_lbq)
    }
  ...
  for (i <- 0 until numLbqEntries)
  {
    ...
    when (reset.asBool || initLBQ.asBool)
    {
      lbq(i).valid           := false.B
      lbq(i).bits.addr       := 0.U
      ...
      when (IsKilledByBranch(io.core.brinfo, lbq(i).bits.uop)) {
        ...
        lbq(i).bits.addr        := io.core.brinfo.target
        lbq(i).bits.mispredict  := io.core.brinfo.mispredict
        lbq(i).bits.taken       := io.core.brinfo.taken

        printf("YH+ [%d] LBQ_EXE lbq(%d) addr: %x taken: %d mispredict: %d\n",
                io.core.tsc_reg, i.U, io.core.brinfo.target, io.core.brinfo.taken, io.core.brinfo.mispredict)
      }
    }
  }

  when (reset.asBool || initLBQ)
  {
    lbq_head := 0.U
    lbq_tail := 0.U
    lbq_commit_head  := 0.U
    ...
    lbq_tail := lbq_commit_head
    lbq_commit_head  := temp_lbq_commit_head 
  } .elsewhen (io.core.brinfo.valid && io.core.brinfo.mispredict && !io.core.exception) {
  ...
  //val regLbqAddr        = Reg(Vec(numLbqEntries, UInt(coreMaxAddrBits.W)))
  //val regLbqTaken       = Reg(Vec(numLbqEntries, Bool()))
  //val regLbqMispredict  = Reg(Vec(numLbqEntries, Bool()))

  for (i <- 0 until numLbqEntries)
  {
    //regLbqAddr(i)       := lbq(i).bits.addr
    //regLbqTaken(i)      := lbq(i).bits.taken
    //regLbqMispredict(i) := lbq(i).bits.mispredict

    //io.core.lbqAddr(i)  := regLbqAddr(i)
    io.core.lbqAddr(i)  := lbq(i).bits.addr
  }
  ...
  io.core.fencei_rdy    := !stq_nonempty && (!brq_nonempty || lrsc_valid) && io.dmem.ordered //yh+
  ...
  //yh+begin
  val block_brq_load_mask    = WireInit(VecInit((0 until numBrqEntries).map(x=>false.B)))
  val p1_block_brq_load_mask = RegNext(block_brq_load_mask)
  val p2_block_brq_load_mask = RegNext(p1_block_brq_load_mask)

  val addr_match_mask = WireInit(VecInit((0 until numBrqEntries).map(x=>false.B)))

  for (i <- 0 until numBrqEntries) { // i is searcher
    for (j <- 0 until numBrqEntries) {

      when (brq(i).valid && brq(j).valid && IsOlder(j.U, i.U, brq_head) &&
              brq(j).bits.state =/= s_done &&
              brq(i).bits.addr.bits === brq(j).bits.addr.bits) {

          printf("YH+ [%d] BRQ Set addr_match_mask(%d) by (%d)\n",
                  io.core.tsc_reg, i.U, j.U)

          addr_match_mask(i) := true.B
      }
    }
  }

  val brq_load_idx = RegNext(AgePriorityEncoder((0 until numBrqEntries).map(i => {
    val e = brq(i).bits
    val block = block_brq_load_mask(i) || p1_block_brq_load_mask(i)
    val addr_match = addr_match_mask(i)
    e.addr.valid && (e.state === s_load) && !e.executed && !block && !addr_match
  }), brq_head))
  val brq_load_e = brq(brq_load_idx)

  //val addr_match = brq_load_e.valid &&
  //                  brq_load_idx =/= brq_head &&
  //                  brq_load_e.bits.addr.bits === brq(brq_head).bits.addr.bits &&
  //                  brq(brq_head).bits.state =/= s_done

  val can_fire_brq_load = widthMap(w => brq_load_e.valid                &&
                                    brq_load_e.bits.state === s_load    &&
                                    !brq_load_e.bits.executed           &&
                                    !io.core.exception                  &&
                                    !lrsc_valid                         &&
                                    !p1_block_brq_load_mask(brq_load_idx) &&
                                    !p2_block_brq_load_mask(brq_load_idx) &&
                                    !addr_match_mask(brq_load_idx)      &&
                                    //!addr_match                         &&
                                    //!(io.core.brinfo.valid && io.core.brinfo.mispredict) &&
                                    (w == memWidth-1).B)


  //val brq_store_e = brq(brq_head)
  val brq_store_e = brq(brq_store_idx)

  val can_fire_brq_store = widthMap(w => brq_store_e.valid              &&
                                    brq_store_e.bits.state === s_store  &&
                                    brq_store_e.bits.committed          &&
                                    !brq_store_e.bits.executed          &&
                                    !io.core.exception                  &&
                                    !lrsc_valid                         &&
                                    (w == memWidth-1).B)

  printf("YH+ [%d] brq_head: %d brq_tail: %d brq_load_idx: %d brq_store_idx: %d valid: %d state: %d executed: %d committed: %d\n",
          io.core.tsc_reg, brq_head, brq_tail, brq_load_idx, brq_store_idx, brq_store_e.valid, brq_store_e.bits.state,
          brq_store_e.bits.executed.asUInt, brq_store_e.bits.committed.asUInt)
  printf("YH+ [%d] can_fire_brq_load: %d can_fire_brq_store: %d stq_commit_e.bits.uop.is_fence: %d stq_commit_e.bits.uop.exception: %d store_needs_order: %d mem_xcpt_valid: %d\n",
          io.core.tsc_reg, can_fire_brq_load.asUInt, can_fire_brq_store.asUInt, stq_commit_e.bits.uop.is_fence, stq_commit_e.bits.uop.exception,
          store_needs_order, mem_xcpt_valid)
  //yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    //yh+begin
    when (will_fire_brq_load(w)) {
      block_brq_load_mask(brq_load_idx) := true.B
    }
    //yh+end
  ...
  //                                brq_store_e.bits.uop, NullMicroOp))

  //val fuzz_exe_paddr = widthMap(w =>
  //                              Mux(will_fire_brq_load (w) ||
  //                                  will_fire_brq_store (w) ,
  //                                  brq_store_e.bits.addr.bits, 0.U))

  val fuzz_exe_uop = widthMap(w =>
                              Mux(will_fire_brq_load(w),
                                  brq_load_e.bits.uop,
                                  Mux(will_fire_brq_store(w),
                                  brq_store_e.bits.uop, NullMicroOp)))

  val fuzz_exe_paddr = widthMap(w =>
                              Mux(will_fire_brq_load(w),
                                  brq_load_e.bits.addr.bits,
                                  Mux(will_fire_brq_store(w),
                                  brq_store_e.bits.addr.bits, 0.U)))
  //yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    //yh+begin
      ...
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_LOAD, brq_idx: %d brq_tail: %d addr: %x\n",
                io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_tail,
                fuzz_exe_paddr(w))
      }
      ...
      brq(brq_load_idx).bits.executed := dmem_req_fire(w)
    } .elsewhen (will_fire_brq_store(w)) {
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_STORE, brq_idx: %d brq_head: %d brq_tail: %d brq_store_idx: %d addr: %x data: %d match_mask: %d\n",
                io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_head, brq_tail, brq_store_idx,
                fuzz_exe_paddr(w), brq_store_e.bits.data.bits + match_mask.reduce(_ + _), match_mask.reduce(_ + _))
      }
      ...
      dmem_req(w).bits.addr  := fuzz_exe_paddr(w)
      dmem_req(w).bits.data  := (new freechips.rocketchip.rocket.StoreGen(
                                    brq_store_e.bits.uop.mem_size, 0.U,
                                    brq_store_e.bits.data.bits + match_mask.reduce(_ + _),
                                    coreDataBytes)).data
      dmem_req(w).bits.uop   := fuzz_exe_uop(w)
      //brq(brq_head).bits.executed := dmem_req_fire(w)
      brq(brq_store_idx).bits.executed := dmem_req_fire(w)
      brq_store_idx          := Mux(dmem_req_fire(w),
                                      WrapInc(brq_store_idx, numBrqEntries),
                                      brq_store_idx)
    //yh+end
    }
  ...
  //yh+begin
  //for (w <- 0 until memWidth) {
  //  when (exe_tlb_uop(w).mem_cmd === rocket.M_XLR) {
  //    printf("YH+ [%d] lr found! M_XLR\n", io.core.tsc_reg)
  //  } .elsewhen (exe_tlb_uop(w).mem_cmd === rocket.M_XSC) {
  //    printf("YH+ [%d] sc found! M_XSC\n", io.core.tsc_reg)
  //  }
  //}
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.nack(w).valid)
    {
      ...
      //yh+begin
        .elsewhen (io.dmem.nack(w).bits.uop.uses_brq)
      {
        when (io.dmem.nack(w).bits.uop.is_htload &&
        ...
        } .elsewhen (!io.dmem.nack(w).bits.uop.is_htload &&
              brq(io.dmem.nack(w).bits.uop.brq_idx).bits.state === s_store) {
          printf("YH+ [%d] BRQ Store nack arrived, brq_idx: %d\n",
                  io.core.tsc_reg, io.dmem.nack(w).bits.uop.brq_idx)
          brq(io.dmem.nack(w).bits.uop.brq_idx).bits.executed := false.B

          when (IsOlder(io.dmem.nack(w).bits.uop.brq_idx, brq_store_idx, brq_head)) {
            brq_store_idx := io.dmem.nack(w).bits.uop.brq_idx
          }
        }
      }
      //yh+end
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      //yh+begin
        .elsewhen (io.dmem.resp(w).bits.uop.uses_brq)
      {
        //when (io.dmem.resp(w).bits.uop.is_htload)
        when (io.dmem.resp(w).bits.uop.is_htload &&
              brq(io.dmem.resp(w).bits.uop.brq_idx).bits.state === s_load)
        {
          ...
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.uop.is_htload := false.B
        ...
        //} .otherwise {
        } .elsewhen (!io.dmem.resp(w).bits.uop.is_htload &&
              brq(io.dmem.resp(w).bits.uop.brq_idx).bits.state === s_store) {
