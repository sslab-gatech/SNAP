...
class BoomFrontendIO(implicit p: Parameters) extends BoomBundle
{
  ...
  val enableLBQ         = Bool()
  val mispredict        = Bool()
  val do_reset          = Bool() // used to reset the bpd state
  //yh+end
}
  ...
  with HasL1ICacheBankedParameters
{
  ...
  fetch_controller.io.enableLBQ := io.cpu.enableLBQ
  fetch_controller.io.mispredict := io.cpu.mispredict
  //yh+end
  ...
  bpdpipeline.io.do_reset := io.cpu.do_reset //yh+
