...
//      ("DTLB miss",   () => io.dmem.perf.tlbMiss),
  ...
  csr.io.fuzzCSR := Cat(io.lsu.brqNonEmpty, io.lsu.brq_full.reduce(_||_), custom_csrs.enableBrDest, custom_csrs.flushLBQ, custom_csrs.enableLBQ, custom_csrs.enableBRQ)
  ...
  for (w <- 0 until coreWidth) {
    ...
    //dis_uops(w).brq_full := io.lsu.brq_full(w) //yh+
  }
