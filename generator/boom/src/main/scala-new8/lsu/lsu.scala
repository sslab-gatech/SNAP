  ...
  with rocket.HasL1HellaCacheParameters
{
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      when (IsKilledByBranch(io.core.brinfo, brq(i).bits.uop)) {
        ...
        brq(i).bits.addr.bits           := bitmap_base |
                                            ((io.core.brinfo.target ^ brq(i).bits.uop.inst_hash ^
                                              temp_prev_hash) & bitmap_size)
        brq(i).bits.state               := s_load
        brq(i).bits.prev_new            := ((io.core.brinfo.target ^ brq(i).bits.uop.inst_hash) >> 1)
  ...
  //    when (mem_xcpt_causes(w) === rocket.Causes.misaligned_load.U) {
  //      printf("YH+ [%d] misalgined_load\n", io.core.tsc_reg)
  //    } .elsewhen (mem_xcpt_causes(w) === rocket.Causes.misaligned_store.U) {
  //      printf("YH+ [%d] misalgined_store\n", io.core.tsc_reg)
  //    } .elsewhen (mem_xcpt_causes(w) === rocket.Causes.load_page_fault.U) {
  //      printf("YH+ [%d] load_page_fault\n", io.core.tsc_reg)
  //    } .elsewhen (mem_xcpt_causes(w) === rocket.Causes.store_page_fault.U) {
  //      printf("YH+ [%d] store_page_fault\n", io.core.tsc_reg)
  //    } .elsewhen (mem_xcpt_causes(w) === rocket.Causes.load_access.U) {
  //      printf("YH+ [%d] load_access\n", io.core.tsc_reg)
  //    } .elsewhen (mem_xcpt_causes(w) === rocket.Causes.store_access.U) {
  //      printf("YH+ [%d] store_access\n", io.core.tsc_reg)
  //    }
  //  }
  //}
  ////yh+end
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      when (io.dmem.resp(w).bits.uop.is_br_or_jmp)
      {
        ...
        when (brq(io.dmem.resp(w).bits.uop.brq_idx).bits.state === s_load)
        //when (io.dmem.resp(w).bits.uop.uses_ldq)
        {
          ...
          .elsewhen (brq(io.dmem.resp(w).bits.uop.brq_idx).bits.state === s_store)
        {
          assert(!io.dmem.resp(w).bits.is_hella)
          //assert(!(io.dmem.resp(w).bits.uop.uses_stq || io.dmem.resp(w).bits.uop.uses_ldq))
