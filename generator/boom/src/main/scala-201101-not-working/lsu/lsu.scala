...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  //val brqNonEmpty = Output(Bool())
  //val brqHead = Output(UInt(brqAddrSz.W))
  //val brqTail = Output(UInt(brqAddrSz.W))
  //val brqCommitHead = Output(UInt(brqAddrSz.W))
  //yh+end
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val brq_load_head     = RegInit(0.U(brqAddrSz.W))
  ...
  val addr_mask = Wire(Vec(numBrqEntries, Bool()))
  for (i <- 0 until numBrqEntries) {

    addr_mask(i) := false.B

    when (brq(i).valid && IsOlder(i.U, brq_load_head, brq_head)) {
      when (brq(i).bits.state =/= s_done &&
            brq(i).bits.addr.bits === brq(brq_load_head).bits.addr.bits) {

        printf("YH+ [%d] brq(%d) conflicting with brq(%d)\n", io.core.tsc_reg, i.U, brq_load_head)
  ...
  //printf("YH+ [%d] brq_full: %d ht_valid: %d brq_idx: %d\n",
  //        io.core.tsc_reg, io.core.brq_full.reduce(_||_),
  //        io.core.brinfo.ht_valid, io.core.brinfo.brq_idx)
  ...
  when (reset.asBool || initBRQ) {
    ...
    brq_load_head := brq_head
    //brq_load_head := temp_brq_commit_head
    //brq_load_head := Mux(IsOlder(temp_brq_commit_head, brq_load_head, brq_head),
    //                      temp_brq_commit_head, brq_load_head)
    //                      //WrapInc(temp_brq_commit_head, numBrqEntries), brq_load_head)
    ...
    when (brq(bidx).valid) {
      ...
      when (!IsKilledByBranch(io.core.brinfo, brq(bidx).bits.uop)) {
        printf("YH+ [%d] Not killed by branch, uopc: %d IsOlder: %d\n",
                io.core.tsc_reg, brq(bidx).bits.uop.uopc,
                IsOlder(io.core.brinfo.brq_idx, brq_load_head, brq_head))

        brq_tail := WrapInc(io.core.brinfo.brq_idx, numBrqEntries)
        brq_load_head := Mux(IsOlder(io.core.brinfo.brq_idx, brq_load_head, brq_head),
                              WrapInc(io.core.brinfo.brq_idx, numBrqEntries), brq_load_head)
        prev_hash := brq(bidx).bits.hash_new

        assert(!brq(bidx).bits.committed)
      } .otherwise {
        printf("YH+ [%d] Killed by branch, uopc: %d IsOlder: %d\n",
                io.core.tsc_reg, brq(bidx).bits.uop.uopc,
                IsOlder(io.core.brinfo.brq_idx, brq_load_head, brq_head))

        brq_tail := io.core.brinfo.brq_idx
        brq_load_head := Mux(IsOlder(io.core.brinfo.brq_idx, brq_load_head, brq_head),
                              io.core.brinfo.brq_idx, brq_load_head)
    ...
    brq_tail := br_enq_idx
    //brq_load_head := brq_load_head
    brq_load_head := Mux(brq(brq_load_head).valid &&
                          ((brq(brq_load_head).bits.state === s_load && brq(brq_load_head).bits.executed) ||
                            brq(brq_load_head).bits.state === s_store ||
                            brq(brq_load_head).bits.state === s_done),
                          WrapInc(brq_load_head, numBrqEntries), brq_load_head)
    brq_commit_head := temp_brq_commit_head
    prev_hash := temp_prev_hash
  }
  ...
  printf("YH+ [%d] brq_head: %d brq_tail: %d brq_load_head: %d brq_commit_head: %d\n",
          io.core.tsc_reg, brq_head, brq_tail, brq_load_head, brq_commit_head)
  ...
  //yh+begin
  val brq_load_e = brq(brq_load_head)
  ...
  val can_fire_brq_load = widthMap(w => brq_load_e.valid                &&
                                    ...
                                    !io.core.exception                  &&
                                    !lrsc_valid                         &&
                                    !addr_mask.reduce (_||_)            &&
                                    !(io.core.brinfo.valid && io.core.brinfo.mispredict) &&
                                    (w == memWidth-1).B)

  //val can_fire_brq_load_temp = widthMap(w => brq_load_e.valid                &&
  //                                  brq_load_e.bits.state === s_load    &&
  //                                  !io.core.exception                  &&
  //                                  !lrsc_valid                         &&
  //                                  !(io.core.brinfo.valid && io.core.brinfo.mispredict) &&
  ...
  //  when (can_fire_brq_load_temp(w) && addr_mask.reduce (_||_)) {
  //    printf("YH+ [%d] Dont send load because addr_mask\n", io.core.tsc_reg)
  //  }
  //}
  ...
  for (w <- 0 until memWidth) {
    ...
    //yh+begin
      ...
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_LOAD, brq_idx: %d brq_load_head: %d brq_tail: %d addr: %x\n",
                io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_load_head, brq_tail,
                fuzz_exe_paddr(w))
      }
      ...
      brq(brq_load_head).bits.executed := dmem_req_fire(w)
      brq_load_head := Mux(dmem_req_fire(w), WrapInc(brq_load_head, numBrqEntries),
                            brq_load_head)
    } .elsewhen (will_fire_brq_store(w)) {
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_STORE, brq_idx: %d brq_head: %d brq_tail: %d addr: %x data: %d match_mask: %d\n",
                io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_head, brq_tail,
                fuzz_exe_paddr(w), brq_exec_e.bits.data.bits + match_mask.reduce(_ + _), match_mask.reduce(_ + _))
      }
    ...
    when (io.dmem.nack(w).valid)
    {
        ...
        .elsewhen (io.dmem.nack(w).bits.uop.uses_brq)
      {
        when (io.dmem.nack(w).bits.uop.is_htload &&
          ...
          //        IsOlder(io.core.brinfo.brq_idx, io.dmem.nack(w).bits.uop.brq_idx, brq_head))) {
    
          when (io.dmem.nack(w).bits.uop.brq_idx === brq_load_head ||
            IsOlder(io.dmem.nack(w).bits.uop.brq_idx, brq_load_head, brq_head)) {
            brq_load_head := io.dmem.nack(w).bits.uop.brq_idx
          }
