...
class FetchBundle(implicit p: Parameters) extends BoomBundle
{
  ...
  val is_br_or_jmp  = Vec(fetchWidth, Bool()) //yh+
}
  ...
  with HasL1ICacheBankedParameters
{
  ...
  for (i <- 0 until fetchWidth) {
    ...
    //yh+begin
    f3_fetch_bundle.is_br_or_jmp(i) := (is_br(i) | is_jal(i) | is_jr(i))
    //yh+end
  }
  ...
  fb.io.enableLBQ := io.enableLBQ
  fb.io.mispredict := io.mispredict
  //yh+end
