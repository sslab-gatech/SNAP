...
class BrResolutionInfo(implicit p: Parameters) extends BoomBundle
{
  ...
  //yh+begin
  val brq_idx    = UInt(brqAddrSz.W)
  val lbq_idx    = UInt(lbqAddrSz.W)
  //yh+end
    ...
    isBranchUnit = isBranchUnit)
{
  ...
  if (isBranchUnit) {
    ...
    //yh+begin
    brinfo.brq_idx        := uop.brq_idx
    brinfo.lbq_idx        := uop.lbq_idx
    //yh+end
    ...
    //yh+begin
    brinfo.ht_valid := io.req.valid && !killed && uop.uses_lbq &&
                        uop.is_br_or_jmp
