...
//yh+begin
class LSUFuzzIO(implicit p: Parameters) extends BoomBundle()(p)
{
  // The "resp" of the maddrcalc is really a "req" to the LSU
  val req         = Flipped(new ValidIO(new FuncUnitResp(xLen)))
...
class LSUBrqIO(implicit p: Parameters) extends BoomBundle()(p)
{
  val valid     = Bool()
  val uop       = new MicroOp
  val hash      = Bits(32.W)
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val lbqTaken = Output(Vec(numLbqEntries, Bool()))
  val lbqMispredict = Output(Vec(numLbqEntries, Bool()))
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (brq_full) {
      printf("YH+ brq_full! br_enq_idx: %d brq_head: %d\n", br_enq_idx, brq_head)
    }

    val dis_br_val = regEnableBRQ && io.core.dis_brq(w).valid && io.core.dis_brq(w).uop.uses_brq

    when (dis_br_val) {
      brq(br_enq_idx).valid             := true.B
      brq(br_enq_idx).bits.uop.brq_idx  := io.core.dis_brq(w).uop.brq_idx
      brq(br_enq_idx).bits.uop.mem_size := 0.U //TODO: need to check
      brq(br_enq_idx).bits.uop.br_mask  := io.core.dis_brq(w).uop.br_mask
      //brq(br_enq_idx).bits.uop.uses_ldq := false.B
      //brq(br_enq_idx).bits.uop.uses_stq := false.B
      brq(br_enq_idx).bits.uop.uses_brq := true.B

      brq(br_enq_idx).bits.addr.valid   := true.B
      //brq(br_enq_idx).bits.addr.bits    := (134217728.U << 4) |
      brq(br_enq_idx).bits.addr.bits    := bitmap_base |
                                            (io.core.dis_brq(w).hash ^ temp_prev_hash & bitmap_size)
      ...
      brq(br_enq_idx).bits.hash         := io.core.dis_brq(w).hash
      brq(br_enq_idx).bits.prev         := temp_prev_hash
      brq(br_enq_idx).bits.forwarded    := false.B

      printf("YH+ [%d] Enqueue BRQ Entry, br_enq_idx: %d brq_head: %d brq_tail: %d uopc: %d hash>>1: %d prev_hash: %d\n",
              io.core.tsc_reg, br_enq_idx, brq_head, brq_tail, io.core.dis_brq(w).uop.uopc,
              io.core.dis_brq(w).hash >> 1, temp_prev_hash);

      //printf("YH+ [%d] uopc: %d\n", io.core.tsc_reg, io.core.dis_brq(w).uop.uopc);
      
      assert (io.core.dis_brq(w).uop.uses_brq, "[lsu] !uses_brq")
    ...
    br_enq_idx = Mux(dis_br_val, WrapInc(br_enq_idx, numBrqEntries),
                                br_enq_idx)
    temp_prev_hash = Mux(dis_br_val, io.core.dis_brq(w).hash >> 1,
                                    temp_prev_hash)
  }
  ...
  for (w <- 0 until coreWidth) {
    val commit_br = regEnableBRQ && io.core.commit.valids(w) &&
                      io.core.commit.uops(w).is_br_or_jmp
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      when (IsKilledByBranch(io.core.brinfo, brq(i).bits.uop)) {
        ...
        printf("YH+ [%d] BRQ S_INIT, brq_idx: %d brq_head: %d brq_tail: %d\n",
  ...
  // Clear TODO
  val clear_br = brq(brq_head).valid && brq(brq_head).bits.state === s_done
  ...
  when (reset.asBool || initBRQ) {
    ...
    when (brq(bidx).valid && !IsKilledByBranch(io.core.brinfo, brq(bidx).bits.uop)) {
      ...
      prev_hash := brq(bidx).bits.hash >> 1
    } .otherwise {
    ...
    //brq_tail := Mux(brq(bidx).valid && !IsKilledByBranch(io.core.brinfo, brq(bidx).bits.uop),
    //                WrapInc(io.core.brinfo.brq_idx, numBrqEntries),
    //                io.core.brinfo.brq_idx)
    //prev_hash := brq(bidx).bits.hash >> 1
  } .otherwise {
    brq_commit_head := temp_brq_commit_head
    brq_commit_hash := temp_brq_commit_hash
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    when (dis_lb_val)
    {
      ...
      printf("YH+ [%d] Enqueue LBQ Entry, lb_enq_idx: %d lbq_head: %d lbq_tail: %d\n",
              io.core.tsc_reg, lb_enq_idx, lbq_head, lbq_tail)
    }
  ...
  {
    val exe_lb_val = fuzz_exe_req.valid && regEnableLBQ &&
                      i.U === fuzz_exe_req.bits.uop.lbq_idx
    ...
    when (reset.asBool || io.core.exception || initLBQ)
    {
      ...
      when (IsKilledByBranch(io.core.brinfo, lbq(i).bits.uop)) {
        ...
        lbq(i).bits.addr.bits   := fuzz_exe_req.bits.addr
        lbq(i).bits.mispredict  := fuzz_exe_mispredict
        lbq(i).bits.taken       := fuzz_exe_taken
      }
    }
  }
  ...
  for (i <- 0 until numLbqEntries)
  {
    ...
    //        io.core.tsc_reg, i.U, io.core.lbqAddr(i), lbq_head, i.U, lbq(lbq_head+i.U).bits.addr)
  }
  ...
  //printf("YH+ [%d] brq_head: %d brq_tail: %d brq_commit_head: %d brq_execute_head: %d brq_exec_e.bits.committed: %d, brq_exec_e.bits.executed: %d\n",
  //        io.core.tsc_reg, brq_head, brq_tail, brq_commit_head, brq_execute_head, brq(brq_execute_head).bits.committed, brq(brq_execute_head).bits.executed);
  ...
  printf("YH+ [%d] brq_exec_e:%d valid: %d state: %d executed: %d committed: %d\n",
          io.core.tsc_reg, brq_head, brq_exec_e.valid, brq_exec_e.bits.state,
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.nack(w).valid)
    {
      ...
      ////yh+begin
      //   .elsewhen (io.dmem.nack(w).bits.uop.is_fuzz_ld)
      //{

      //  assert(io.dmem.nack(w).bits.uop.uses_brq && io.dmem.nack(w).bits.uop.is_fuzz_ld)
      //  assert(brq(io.dmem.nack(w).bits.uop.brq_idx).bits.executed)

      //  brq(io.dmem.nack(w).bits.uop.brq_idx).bits.executed := false.B
      //}
      //   .elsewhen (io.dmem.nack(w).bits.uop.is_fuzz_st)
      //{
      //  printf("YH+ [%d] Received io.dmem.nack is_fuzz_st brq_idx: %d\n",
      //          io.core.tsc_reg, io.dmem.nack(w).bits.uop.brq_idx)

      //  assert(io.dmem.nack(w).bits.uop.uses_brq && io.dmem.nack(w).bits.uop.is_fuzz_st)
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      when (io.dmem.resp(w).bits.uop.is_br_or_jmp)
      {
        when (io.dmem.resp(w).bits.uop.is_fuzz_ld)
        {
          ...
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.uop.is_fuzz_ld := false.B
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.uop.is_fuzz_st := true.B
          ...
          .elsewhen (io.dmem.resp(w).bits.uop.is_fuzz_st)
        {
          assert(!io.dmem.resp(w).bits.is_hella)
          //assert(!(io.dmem.resp(w).bits.uop.uses_stq || io.dmem.resp(w).bits.uop.uses_ldq))

          printf("YH+ [%d] Store resp arrived! brq_idx: %d data: %d\n",
                  io.core.tsc_reg, io.dmem.resp(w).bits.uop.brq_idx,
                  io.dmem.resp(w).bits.data)
      ...
      ////yh+begin
      //}
      //  .elsewhen (io.dmem.resp(w).bits.uop.is_fuzz_ld)
      //{
      //  assert(!io.dmem.resp(w).bits.is_hella)
      //  assert(!(io.dmem.resp(w).bits.uop.uses_stq || io.dmem.resp(w).bits.uop.uses_ldq))
    ...
    //yh-when (io.core.exe(w).iresp.valid && io.core.exe(w).iresp.bits.uop.uses_ldq) {
    ...
    //yh-} .elsewhen (io.core.exe(w).fresp.valid && io.core.exe(w).fresp.bits.uop.uses_ldq) {
  ...
  when (hella_state === h_ready) {
    ...
    for (w <- 0 until memWidth) {
      when (io.dmem.resp(w).valid && io.dmem.resp(w).bits.is_hella) {
        ...
        printf("YH+ [%d] YH+ nack valid & is_hella\n", io.core.tsc_reg) //yh+
