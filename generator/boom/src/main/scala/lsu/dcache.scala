  ...
  with HasBoomCoreParameters
{
  ...
  val s1_valid = widthMap(w =>
                 RegNext(s0_valid(w)                                     &&
                         ...
                         !(io.lsu.exception && (s0_req(w).uop.uses_ldq || s0_req(w).uop.is_htload))   && //yh+
                         !(s2_store_failed && io.lsu.req.fire() && s0_req(w).uop.uses_stq),
                         //!(s2_store_failed && io.lsu.req.fire() && (s0_req(w).uop.uses_stq && !s0_req(w).uop.is_br_or_jmp)), //yh+
                         init=false.B))
  for (w <- 0 until memWidth)
  ...
  val s2_valid = widthMap(w =>
                  RegNext(s1_valid(w) &&
                         ...
                         !(io.lsu.exception && (s1_req(w).uop.uses_ldq || s1_req(w).uop.is_htload)) && //yh+
                         !(s2_store_failed && (s1_type === t_lsu) && s1_req(w).uop.uses_stq)))
                         //!(s2_store_failed && (s1_type === t_lsu) && (s1_req(w).uop.uses_stq && !s1_req(w).uop.is_br_or_jmp)))) //yh+
  for (w <- 0 until memWidth)
  ...
  //s2_store_failed := s2_valid(0) && s2_nack(0) && s2_send_nack(0) && (s2_req(0).uop.uses_stq && !s2_req(0).uop.is_br_or_jmp) //yh+
  ...
  for (w <- 0 until memWidth) {
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            ...
                            !(io.lsu.exception && (s2_req(w).uop.uses_ldq || s2_req(w).uop.is_htload))   && //yh+
  ...
  //yh+begin
  val regEnableDebug   = Reg(Bool())
  val initDebug        = Reg(Bool())	

  regEnableDebug := io.lsu.enableDebug
  initDebug := io.lsu.enableDebug & !regEnableDebug

  val cache_hit_cnt = RegInit(0.U(xLen.W))
  val cache_miss_cnt = RegInit(0.U(xLen.W))

  var temp_cache_hit_cnt = cache_hit_cnt
  var temp_cache_miss_cnt = cache_miss_cnt

  for (w <- 0 until memWidth) {
    val is_cache_hit = s2_valid(w)          &&
                        s2_hit(w)           &&
                        !IsKilledByBranch(io.lsu.brinfo, s2_req(w).uop)     &&
                        (s2_req(w).uop.uses_ldq || s2_req(w).uop.uses_stq)  &&
                        !(io.lsu.exception && (s2_req(w).uop.uses_ldq || s2_req(w).uop.is_htload))

    val is_cache_miss = s2_valid(w)         &&
                        !s2_hit(w)          &&
                        !IsKilledByBranch(io.lsu.brinfo, s2_req(w).uop)     &&
                        (s2_req(w).uop.uses_ldq || s2_req(w).uop.uses_stq)  &&
                        !(io.lsu.exception && (s2_req(w).uop.uses_ldq || s2_req(w).uop.is_htload))

    temp_cache_hit_cnt = Mux(io.lsu.enableDebug && is_cache_hit, temp_cache_hit_cnt + 1.U,
                              temp_cache_hit_cnt)

    temp_cache_miss_cnt = Mux(io.lsu.enableDebug && is_cache_miss, temp_cache_miss_cnt + 1.U,
                              temp_cache_miss_cnt)

    when (io.lsu.enableDebug && is_cache_hit) {
      printf("YH+ cache_hit_cnt: %d\n", temp_cache_hit_cnt)
    }

    when (io.lsu.enableDebug && is_cache_miss) {
      printf("YH+ cache_miss_cnt: %d\n", temp_cache_miss_cnt)
    }
  }

  when (initDebug) {
    printf("YH+ Reset miss_cnt hit: %d miss: %d\n", io.lsu.regCacheHitCnt, io.lsu.regCacheMissCnt)

    cache_hit_cnt := io.lsu.regCacheHitCnt
    cache_miss_cnt := io.lsu.regCacheMissCnt
  } .elsewhen(regEnableDebug) {
    cache_hit_cnt := temp_cache_hit_cnt
    cache_miss_cnt := temp_cache_miss_cnt
  }

  io.lsu.cacheHitCnt := cache_hit_cnt
  io.lsu.cacheMissCnt := cache_miss_cnt
  //yh+end
  ...
  for (w <- 0 until memWidth) {
    io.lsu.resp(w).valid := resp(w).valid &&
                            ...
                            !(io.lsu.exception && (resp(w).bits.uop.uses_ldq || resp(w).bits.uop.is_htload)) && //yh+
