...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val regCacheHitCnt = Output(UInt(xLen.W))
  val regCacheMissCnt = Output(UInt(xLen.W))
  val cacheHitCnt = Input(UInt(xLen.W))
  val cacheMissCnt = Input(UInt(xLen.W))
  //yh+end
}
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val cacheHitCnt = Output(UInt(xLen.W))
  val cacheMissCnt = Output(UInt(xLen.W))
  //yh+end
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val regCacheHitCnt   = Reg(UInt(xLen.W))
  val regCacheMissCnt  = Reg(UInt(xLen.W))
  ...
  regCacheHitCnt := io.core.ht_config.cacheHitCnt
  regCacheMissCnt := io.core.ht_config.cacheMissCnt
  io.dmem.enableDebug := io.core.ht_config.enableDebug
  io.dmem.regCacheHitCnt := regCacheHitCnt
  io.dmem.regCacheMissCnt := regCacheMissCnt
  io.core.cacheHitCnt := io.dmem.cacheHitCnt
  io.core.cacheMissCnt := io.dmem.cacheMissCnt
