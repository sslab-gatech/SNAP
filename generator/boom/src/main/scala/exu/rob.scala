  ...
  )(implicit p: Parameters)  extends BoomBundle
{
  ...
  //yh+begin
  //val enableDebug = Input(Bool())
  //val regNumCommittedInst = Input(UInt(xLen.W))
  //val regNumUsesBrqInst = Input(UInt(xLen.W))
  //val numCommittedInst = Output(UInt(xLen.W))
  //val numUsesBrqInst = Output(UInt(xLen.W))
  //yh+end
}
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
  ...
  //val numCommittedInst = RegInit(0.U(xLen.W))
  //val numUsesBrqInst   = RegInit(0.U(xLen.W))

  //io.numCommittedInst := numCommittedInst
  //io.numUsesBrqInst := numUsesBrqInst

  //regEnableDebug := io.enableDebug
  //initDebug := io.enableDebug & !regEnableDebug

  //when (initDebug) {
  //  printf("YH+ Reset numInst committed_inst: %d uses_brq_inst: %d\n",
  //          io.regNumCommittedInst, io.regNumUsesBrqInst)

  //  numCommittedInst := io.regNumCommittedInst
  //  numUsesBrqInst := io.regNumUsesBrqInst
  //} .elsewhen (regEnableDebug && finished_committing_row) {
  //  printf("YH+ numCommittedInst: %d\n", numCommittedInst + 1.U)

  //  numCommittedInst := numCommittedInst + 1.U

  //  when (rob_uop(rob_head).uses_brq) {
  //    printf("YH+ numUsesBrqInst: %d\n", numUsesBrqInst + 1.U)

  //    numUsesBrqInst := numUsesBrqInst + 1.U
  //  }
  //}
  //yh+end
