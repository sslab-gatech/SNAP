...
//      ("DTLB miss",   () => io.dmem.perf.tlbMiss),
  ...
  csr.io.fuzzCSR := Cat(custom_csrs.flushLBQ, custom_csrs.enableLBQ, custom_csrs.enableBRQ)
  ...
  csr.io.cacheHitCnt := io.lsu.cacheHitCnt
  csr.io.cacheMissCnt := io.lsu.cacheMissCnt
  //csr.io.numCommittedInst := rob.io.numCommittedInst
  //csr.io.numUsesBrqInst := rob.io.numUsesBrqInst
  //yh+end
  ...
  //yh+begin
  io.ifu.mispredict := br_unit.brinfo.mispredict
  //yh+end
  ...
  io.lsu.ht_config.enableDebug := custom_csrs.enableDebug
  ...
  io.lsu.ht_config.cacheHitCnt := custom_csrs.cacheHitCnt
  io.lsu.ht_config.cacheMissCnt := custom_csrs.cacheMissCnt
  //rob.io.regNumCommittedInst := custom_csrs.numCommittedInst
  //rob.io.regNumUsesBrqInst := custom_csrs.numUsesBrqInst
  io.ifu.startAddr := custom_csrs.startAddr
  io.ifu.endAddr := custom_csrs.endAddr
  io.ifu.do_reset := custom_csrs.do_reset
  //io.ifu.enableBRQ := custom_csrs.enableBRQ
  //io.ifu.enableLBQ := custom_csrs.enableLBQ
  //rob.io.enableDebug := custom_csrs.enableDebug
  //rob.io.enableDebug := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableDebug)
  io.ifu.enableBRQ := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableBRQ)
  io.ifu.enableLBQ := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableLBQ)
  ...
  val dis_hazards = (0 until coreWidth).map(w =>
                      ...
                      || io.lsu.brq_full(w) && dis_uops(w).uses_brq //yh+
