...
// DOC include start: BOOM Parameters
  ...
  numBrqEntries: Int = 24, //yh+
  numLbqEntries: Int = 32, //yh+
...
) extends freechips.rocketchip.tile.CoreParams
{
  ...
  //val lrscCycles: Int = 511 // worst case is 14 mispredicted branches + slop //yh+
...
class BoomCustomCSRs(implicit p: Parameters) extends freechips.rocketchip.tile.CustomCSRs
  ...
  //yh+begin
  override def baseCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(baseCSRId, mask, Some(init)))
  }

  override def sizeCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0xFFFF)
    Some(CustomCSR(sizeCSRId, mask, Some(init)))
  }

  override def fuzzCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    //val mask = BigInt(0x7)
    val init = BigInt(0x0)
    //val init = BigInt(0x3)
    Some(CustomCSR(fuzzCSRId, mask, Some(init)))
  }

  override def startAddrCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(startAddrCSRId, mask, Some(init)))
  }

  override def endAddrCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x7FFFFFFF)
    Some(CustomCSR(endAddrCSRId, mask, Some(init)))
  }

  override def prevCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(prevCSRId, mask, Some(init)))
  }

  override def brqFwdCntCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(brqFwdCntCSRId, mask, Some(init)))
  }

  override def brqCommitCntCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(brqCommitCntCSRId, mask, Some(init)))
  }

  override def cacheHitCntCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(cacheHitCntCSRId, mask, Some(init)))
  }

  override def cacheMissCntCSR = {
    val params = tileParams.core.asInstanceOf[BoomCoreParams]
    var mask = BigInt(0)
    mask = ~mask
    val init = BigInt(0x0)
    Some(CustomCSR(cacheMissCntCSRId, mask, Some(init)))
  }

  //override def numCommittedInstCSR = {
  //  val params = tileParams.core.asInstanceOf[BoomCoreParams]
  //  var mask = BigInt(0)
  //  mask = ~mask
  //  val init = BigInt(0x0)
  //  Some(CustomCSR(numCommittedInstCSRId, mask, Some(init)))
  //}

  //override def numUsesBrqInstCSR = {
  //  val params = tileParams.core.asInstanceOf[BoomCoreParams]
  //  var mask = BigInt(0)
  //  mask = ~mask
  //  val init = BigInt(0x0)
  //  Some(CustomCSR(numUsesBrqInstCSRId, mask, Some(init)))
  //}
  //yh+end
}
...
trait HasBoomCoreParameters extends freechips.rocketchip.tile.HasCoreParameters
{
  ...
  val numBrqEntries = boomParams.numBrqEntries       // number of BRQ entries //yh+
  val numLbqEntries = boomParams.numLbqEntries       // number of LBQ entries //yh+
  ...
  val brqAddrSz       = log2Ceil(numBrqEntries) //yh+
  val lbqAddrSz       = log2Ceil(numLbqEntries) //yh+
  ...
  require ((numBrqEntries-1) > coreWidth) //yh+
  require ((numLbqEntries-1) > coreWidth) //yh+
