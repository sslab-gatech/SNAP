  ...
  with HasBoomCoreParameters
{
  ...
  val s1_valid = widthMap(w =>
                 RegNext(s0_valid(w)                                     &&
                         ...
                         //!(io.lsu.exception && (s0_req(w).uop.uses_ldq || s0_req(w).uop.is_fuzz_ld))   && //yh+
                         //!(io.lsu.exception && (s0_req(w).uop.uses_ldq && !s0_req(w).uop.is_br_or_jmp))   && //yh+
  ...
  val s2_valid = widthMap(w =>
                  RegNext(s1_valid(w) &&
                         ...
                         //!(io.lsu.exception && (s1_req(w).uop.uses_ldq || s1_req(w).uop.is_fuzz_ld)) && //yh+
                         //!(io.lsu.exception && (s1_req(w).uop.uses_ldq && !s1_req(w).uop.is_br_or_jmp)) && //yh+
  ...
  //yh-val s2_hit = widthMap(w => (s2_tag_match(w) && s2_has_permission(w) && s2_hit_state(w) === s2_new_hit_state(w) && !mshrs.io.block_hit(w)) || s2_type.isOneOf(t_replay, t_wb))
  val s2_hit = widthMap(w => (s2_tag_match(w) && s2_hit_state(w) === s2_new_hit_state(w) && !mshrs.io.block_hit(w)) || s2_type.isOneOf(t_replay, t_wb)) //yh+
  ...
  for (w <- 0 until memWidth) {
    when (s2_valid(w)                            &&
      s2_type === t_lsu                          &&
      !s2_hit(w)                                 &&
      //yh-!(s2_has_permission(w) && s2_tag_match(w)) &&
      !(s2_tag_match(w)) && //yh+
  ...
  s2_store_failed := s2_valid(0) && s2_nack(0) && s2_send_nack(0) && (s2_req(0).uop.uses_stq && !s2_req(0).uop.is_br_or_jmp)
  ...
  for (w <- 0 until memWidth) {
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            ...
                            //!(io.lsu.exception && (s2_req(w).uop.uses_ldq || s2_req(w).uop.is_fuzz_ld))   && //yh+
                            //!(io.lsu.exception && (s2_req(w).uop.uses_ldq && !s2_req(w).uop.is_br_or_jmp))   && //yh+
  ...
  for (w <- 0 until memWidth) {
    io.lsu.resp(w).valid := resp(w).valid &&
                            ...
                            //!(io.lsu.exception && (resp(w).bits.uop.uses_ldq || resp(w).bits.uop.is_fuzz_ld)) && //yh+
                            //!(io.lsu.exception && (resp(w).bits.uop.uses_ldq && !resp(w).bits.uop.is_br_or_jmp)) && //yh+
