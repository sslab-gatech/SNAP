  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (dis_br_val) {
      ...
      brq(br_enq_idx).bits.tlb_miss     := false.B
      ...
      printf("YH+ [%d] Enqueue BRQ Entry, br_enq_idx: %d brq_head: %d brq_tail: %d uopc: %d hash>>1: %d prev_hash: %d debug_pc: %x\n",
              ...
              (io.core.dis_uops(w).bits.debug_pc ^ io.core.dis_uops(w).bits.hash)(31,0) >> 1,
    ...
    br_enq_idx = Mux(dis_br_val, WrapInc(br_enq_idx, numBrqEntries),
                                br_enq_idx)
    temp_prev_hash = Mux(dis_br_val, (io.core.dis_uops(w).bits.debug_pc ^ io.core.dis_uops(w).bits.hash)(31,0) >> 1,
                                    temp_prev_hash)
  }
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      //} .elsewhen (brq(i).bits.state === s_init) {
        ...
        brq(i).bits.uop.ctrl.is_load    := true.B
        brq(i).bits.uop.ctrl.is_sta     := false.B
        brq(i).bits.uop.ctrl.is_std     := false.B
  ...
  for (w <- 0 until memWidth) {
    ...
    will_fire_brq_load      (w) := lsu_sched(can_fire_brq_load      (w) , true , true , false, false) //yh+
    will_fire_brq_store     (w) := lsu_sched(can_fire_brq_store     (w) , true , true , false, false) //yh+
  ...
  val exe_tlb_uop = widthMap(w =>
                    ...
                    //yh+begin
                    Mux(will_fire_brq_load      (w) ||
                        will_fire_brq_store     (w)  , brq_exec_e.bits.uop,
                    //yh+end
                                                       NullMicroOp))))))
  ...
  val exe_tlb_vaddr = widthMap(w =>
                    ...
                    //yh+begin
                    Mux(will_fire_brq_load      (w) ||
                        will_fire_brq_store     (w)  , brq_exec_e.bits.addr.bits,
                    //yh+end
                                                       0.U)))))))
  ...
  for (w <- 0 until memWidth) {
    ...
    when (mem_xcpt_valids(w))
    {
      assert(RegNext(will_fire_load_incoming(w) || will_fire_stad_incoming(w) || will_fire_sta_incoming(w) ||
        will_fire_load_retry(w) || will_fire_sta_retry(w) || will_fire_brq_load(w) || will_fire_brq_store(w))) //yh+
      ...
      //yh+begin
      when (mem_xcpt_uops(w).is_br_or_jmp)
      {
      }
  ...
  for (w <- 0 until memWidth) {
    when (will_fire_load_incoming(w) || will_fire_stad_incoming(w) || will_fire_load_retry(w) || will_fire_sta_retry(w)) {
      printf("YH+ [%d] exe_tlb_vaddr: %x will_fire_brq_load: %d will_fire_brq_store: %d\n",
              io.core.tsc_reg, exe_tlb_vaddr(w), will_fire_brq_load(w), will_fire_brq_store(w))
    }
    printf("YH+ [%d] exe_tlb_miss(w): %d\n", io.core.tsc_reg, exe_tlb_miss(w))
  }
  ...
  for (w <- 0 until memWidth) {
    ...
    //yh+begin
      ...
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_LOAD, brq_idx: %d brq_head: %d brq_tail: %d paddr: %x vaddr: %x miss: %d\n",
                io.core.tsc_reg, exe_tlb_uop(w).brq_idx, brq_head, brq_tail,
                exe_tlb_paddr(w), brq_exec_e.bits.addr.bits, exe_tlb_miss(w))
                //io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_head, brq_tail,
                //fuzz_exe_paddr(w))
      }

      printf("YH+ [%d] dmem_req_fire(w): %d exe_tlb_miss(w): %d exe_tlb_uncacheable(w): %d exe_tlb_paddr(w): %x\n",
              io.core.tsc_reg, dmem_req_fire(w), exe_tlb_miss(w), exe_tlb_uncacheable(w), exe_tlb_paddr(w))


      assert(exe_tlb_uop(w).uopc === uopLD)
      ...
      //yh-brq(brq_head).bits.tlb_miss := exe_tlb_miss(w)
      brq(brq_head).bits.tlb_miss := !dmem_req_fire(w)
    } .elsewhen (will_fire_brq_store(w)) {
      when (dmem_req_fire(w)) {
        printf("YH+ [%d] BRQ S_STORE, brq_idx: %d brq_head: %d brq_tail: %d paddr: %x vaddr: %x data: %d miss: %d\n",
                io.core.tsc_reg, exe_tlb_uop(w).brq_idx, brq_head, brq_tail,
                exe_tlb_paddr(w), brq_exec_e.bits.addr.bits, brq_exec_e.bits.data.bits + match_mask.reduce(_ + _), exe_tlb_miss(w))
                //io.core.tsc_reg, fuzz_exe_uop(w).brq_idx, brq_head, brq_tail,
                //fuzz_exe_paddr(w), brq_exec_e.bits.data.bits + match_mask.reduce(_ + _))
      }

      assert(exe_tlb_uop(w).uopc === uopSTA)
      ...
      dmem_req(w).bits.data  := (new freechips.rocketchip.rocket.StoreGen(
                                    ...
                                    coreDataBytes)).data
      brq(brq_head).bits.executed := dmem_req_fire(w)
      //brq(brq_head).bits.tlb_miss := exe_tlb_miss(w)
      brq(brq_head).bits.tlb_miss := !dmem_req_fire(w)
    //yh+end
    }
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      when (io.dmem.resp(w).bits.uop.is_br_or_jmp)
      {
        when (io.dmem.resp(w).bits.uop.uses_ldq)
        {
          ...
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.uop.ctrl.is_load := false.B
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.uop.ctrl.is_sta := true.B
          ...
          brq(io.dmem.resp(w).bits.uop.brq_idx).bits.tlb_miss := false.B
