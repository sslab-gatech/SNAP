...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    //yh-when (io.lxcpt.valid && MatchBank(GetBankIdx(io.lxcpt.bits.uop.rob_idx))) {
    //yh-  rob_exception(GetRowIdx(io.lxcpt.bits.uop.rob_idx)) := true.B
    //yh-  when (io.lxcpt.bits.cause =/= MINI_EXCEPTION_MEM_ORDERING) {
    //yh-    // In the case of a mem-ordering failure, the failing load will have been marked safe already.
    //yh-    assert(rob_unsafe(GetRowIdx(io.lxcpt.bits.uop.rob_idx)),
    //yh-      "An instruction marked as safe is causing an exception")
    //yh-  }
    //yh-}
  ...
  //yh-assert (!(empty && r_xcpt_val),
  //yh-  "ROB is empty, but believes it has an outstanding exception.")
