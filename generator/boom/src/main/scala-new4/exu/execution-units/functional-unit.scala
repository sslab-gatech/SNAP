...
class BrResolutionInfo(implicit p: Parameters) extends BoomBundle
{
  ...
  val brq_idx    = UInt(brqAddrSz.W) //yh+
  val lbq_idx    = UInt(lbqAddrSz.W) //yh+
  //val brq_full   = Bool() //yh+
    ...
    isBranchUnit = isBranchUnit)
{
  ...
  if (isBranchUnit) {
    ...
    brinfo.brq_idx        := uop.brq_idx //yh+
    brinfo.lbq_idx        := uop.lbq_idx //yh+
    //brinfo.brq_full       := uop.brq_full //yh+
    ...
    //yh+begin
    brinfo.ht_valid := io.req.valid && !killed && uop.uses_brq &&
                        uop.is_br_or_jmp
                        //((uop.is_jump && !mispredict) || (uop.is_br_or_jmp && !uop.is_jump))
