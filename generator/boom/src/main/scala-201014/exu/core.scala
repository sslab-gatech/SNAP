...
//      ("DTLB miss",   () => io.dmem.perf.tlbMiss),
  ...
  val brq_full = Reg(Bool())
  brq_full := brq_full || io.lsu.brq_full.reduce(_||_)

  //yh+begin
  csr.io.brqPrev := io.lsu.brqPrev
  csr.io.lbqHead := io.lsu.lbqHead
  csr.io.lbqAddr := io.lsu.lbqAddr
  csr.io.lbqStatus := io.lsu.lbqStatus
  csr.io.baseAddrOfBitmap := custom_csrs.baseAddrOfBitmap
  csr.io.sizeOfBitmap := custom_csrs.sizeOfBitmap
  csr.io.fuzzCSR := Cat(io.lsu.brqNonEmpty, brq_full, custom_csrs.enableBrDest, custom_csrs.flushLBQ, custom_csrs.enableLBQ, custom_csrs.enableBRQ)
  csr.io.startAddr := custom_csrs.startAddr
  csr.io.endAddr := custom_csrs.endAddr
  csr.io.brqFwdCnt := io.lsu.brqFwdCnt
  csr.io.brqCommitCnt := io.lsu.brqCommitCnt
  csr.io.brqHead := io.lsu.brqHead
  csr.io.brqTail := io.lsu.brqTail
  csr.io.brqCommitHead := io.lsu.brqCommitHead
  //yh+end
  ...
  ////yh+begin 
  //val is_amo_held = RegInit(false.B)
  //var temp_is_amo_held = is_amo_held
  ////yh+end
  ...
  for (w <- 0 until coreWidth) {
    ...
    ////yh+begin
    //dec_uops(w).uses_brq := decode_units(w).io.deq.uop.uses_brq & !temp_is_amo_held
    //temp_is_amo_held = Mux(decode_units(w).io.deq.uop.is_amo && decode_units(w).io.deq.uop.inst(26) &&
    //                      !decode_units(w).io.deq.uop.inst(27)/* amo_held */, true.B,
    //                      Mux(decode_units(w).io.deq.uop.is_amo && decode_units(w).io.deq.uop.inst(25) /* amo_rl */, false.B,
    //                      temp_is_amo_held))
    ////yh+end
  }
  ...
  //for (w <- 0 until coreWidth) {
  //  when (dec_fire(w)) {
  //    when (dec_uops(w).is_amo) {
  //      printf("YH+ [%d] is_amo found! inst(26): %d inst(25): %d\n", debug_tsc_reg, dec_uops(w).inst(26), dec_uops(w).inst(25))
  //    }

  //    when (dec_uops(w).is_amo && dec_uops(w).inst(26)) {
  //      printf("YH+ [%d] amo_aq found! is_amo_held: %d\n", debug_tsc_reg, is_amo_held)
  //    } .elsewhen (dec_uops(w).is_amo && dec_uops(w).inst(25)) {
  //      printf("YH+ [%d] amo_rl found! is_amo_held: %d\n", debug_tsc_reg, is_amo_held)
  //    }

  //    when (dec_uops(w).is_br_or_jmp) {
  //      printf("YH+ [%d] uses_brq: %d\n", debug_tsc_reg, dec_uops(w).uses_brq)
  //    }
  //  }
  //}
  //yh+end

  //is_amo_held := temp_is_amo_held //yh+
  ...
  val dec_hazards = (0 until coreWidth).map(w =>
                      ...
                      || dec_xcpt_stall
                      || branch_mask_full(w)
                      //|| io.lsu.brq_full(w) && dec_uops(w).is_br_or_jmp && dec_uops(w).uses_brq //yh+
                      || flush_ifu))
  ...
  io.lsu.ht_config.base := custom_csrs.baseAddrOfBitmap
  io.lsu.ht_config.size := custom_csrs.sizeOfBitmap
  //io.lsu.ht_config.enableBRQ := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableBRQ)
  //io.lsu.ht_config.enableBRQ := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableBRQ)
  io.lsu.ht_config.enableBRQ := custom_csrs.enableBRQ
  io.lsu.ht_config.enableLBQ := custom_csrs.enableLBQ
  io.lsu.ht_config.flushLBQ := custom_csrs.flushLBQ
  io.lsu.ht_config.prev := custom_csrs.prev
  io.lsu.ht_config.brqFwdCnt := custom_csrs.brqFwdCnt
  io.lsu.ht_config.brqCommitCnt := custom_csrs.brqCommitCnt
  io.ifu.startAddr := custom_csrs.startAddr
  io.ifu.endAddr := custom_csrs.endAddr
  //io.ifu.enableBRQ := Mux(csr.io.status.prv > 0.U, false.B, custom_csrs.enableBRQ)
  io.ifu.enableBRQ := custom_csrs.enableBRQ
  io.ifu.enableBrDest := custom_csrs.enableBrDest

  //printf("YH+ baseAddrOfBitmap: %x startAddr: %x endAddr: %x\n",
  //            custom_csrs.baseAddrOfBitmap, custom_csrs.startAddr, custom_csrs.endAddr)
  //yh+end
  ...
  val dis_hazards = (0 until coreWidth).map(w =>
                      ...
                      || io.lsu.brq_full(w) && dis_uops(w).is_br_or_jmp && dis_uops(w).uses_brq //yh+
  ...
  for (w <- 0 until coreWidth) {
    ...
    dis_uops(w).brq_idx := io.lsu.dis_brq_idx(w) //yh+
    dis_uops(w).lbq_idx := io.lsu.dis_lbq_idx(w) //yh+
  }
