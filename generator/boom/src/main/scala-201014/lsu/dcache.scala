  ...
  with HasBoomCoreParameters
{
  ...
  ////yh+begin
  //val is_brq_req = widthMap(w => s2_valid(w)            &&
  //                          s2_req(w).uop.is_br_or_jmp)

  //when (is_brq_req.reduce(_||_) && lrsc_count > 0.U) {
  //  printf("brq_req is resetting lrsc_count\n")
  //  lrsc_count := (lrscCycles - 1).U
  //}
  ...
  //for (w <- 0 until memWidth) {
  //  printf("w:%d s2_valid: %d s2_hit: %d s2_nack: %d is_br_or_jmp: %d\n",
  //          w.U, s2_valid(w), s2_hit(w), s2_nack(w), s2_req(w).uop.is_br_or_jmp)
  //}
  //yh+end
