  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val is_reserved      = Reg(Bool()) // Halt bitmap update during set
  ...
  for (w <- 0 until coreWidth) {
    val commit_br = io.core.commit.valids(w) &&
                      io.core.commit.uops(w).uses_brq && io.core.commit.uops(w).is_br_or_jmp

    var bidx = io.core.commit.uops(w).brq_idx

    when (commit_br) {
      printf("YH+ [%d] BRQ COMMIT, brq_idx: %d target: %x addr: %x prev_used: %x prev_new: %x\n",
  ...
  when (io.core.brinfo.ht_valid) {
    assert(brq(io.core.brinfo.brq_idx).valid)
  }
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    //temp_prev_hash = Mux(exe_br_val, ((io.core.brinfo.target ^ brq(i).bits.uop.inst_hash) >> 1),
    //                                temp_prev_hash)
  }
  ...
  for (i <- 0 until numLbqEntries)
  {
    regLbqAddr(i)       := lbq(i).bits.addr.bits
    regLbqTaken(i)      := lbq(i).bits.taken
    regLbqMispredict(i) := lbq(i).bits.mispredict
    //printf("YH+ [%d] lbq(%d) addr: %x taken: %d mispredict: %d\n",
    //        io.core.tsc_reg, i.U, lbq(i).bits.addr.bits, lbq(i).bits.taken, lbq(i).bits.mispredict)
  ...
  io.core.fencei_rdy    := !stq_nonempty && (!brq_nonempty || is_reserved) && io.dmem.ordered //yh+
  ...
  //val can_fire_brq_load_temp = widthMap(w => brq_exec_e.valid           &&
  //                                  brq_exec_e.bits.state === s_load    &&
  //                                  brq_exec_e.bits.committed           &&
  //                                  !brq_exec_e.bits.executed           &&
  //                                  !io.core.exception                  &&
  ...
  //val can_fire_brq_store_temp = widthMap(w => brq_exec_e.valid          &&
  //                                  brq_exec_e.bits.state === s_store   &&
  //                                  brq_exec_e.bits.committed           &&
  //                                  !brq_exec_e.bits.executed           &&
  //                                  !io.core.exception                  &&
  ...
  //  when (can_fire_brq_load_temp(w) && is_reserved) {
  //    printf("YH+ [%d] Could've fired brq_load if not is_reserved is set\n", io.core.tsc_reg)
  //  } .elsewhen (can_fire_brq_store_temp(w) && is_reserved) {
  //    printf("YH+ [%d] Could've fired brq_store if not is_reserved is set\n", io.core.tsc_reg)
  //  }
  //}
  ...
  for (i <- 0 until numLdqEntries)
  {
    when (ldq(i).valid)
    {
      ...
      //yh+begin
      val is_lr = ldq(i).bits.uop.mem_cmd === rocket.M_XLR &&
                  !(ldq(i).bits.uop.inst(26) & ldq(i).bits.uop.inst(25))

      when (is_lr && IsKilledByBranch(io.core.brinfo, ldq(i).bits.uop))
      {
        is_reserved := false.B
      }
      //yh+end
  ...
  when (reset.asBool) {
    ...
    val is_sc = stq(stq_head).bits.uop.mem_cmd === rocket.M_XSC

    for (w <- 0 until coreWidth) {
      val dis_ld_val = io.core.dis_uops(w).valid && io.core.dis_uops(w).bits.uses_ldq && !io.core.dis_uops(w).bits.exception
      val is_lr = io.core.dis_uops(w).bits.mem_cmd === rocket.M_XLR &&
                  !(io.core.dis_uops(w).bits.inst(26) & io.core.dis_uops(w).bits.inst(25))

      when (dis_ld_val && is_lr) {
        printf("YH+ [%d] Set is_reserved\n", io.core.tsc_reg)
        is_reserved := true.B
      }
    }

    when (clear_store && is_sc) {
      printf("YH+ [%d] Clear is_reserved\n", io.core.tsc_reg)
      is_reserved := false.B
    }
  }

  ////var temp_is_reserved = is_reserved

  //when (reset.asBool) {
  //  is_reserved := false.B
  //  //temp_is_reserved = false.B
  //} .otherwise {
  //  val is_sc = stq(stq_head).bits.uop.mem_cmd === rocket.M_XSC

  //  for (w <- 0 until coreWidth) {
  //    val commit_load  = io.core.commit.valids(w) && io.core.commit.uops(w).uses_ldq
  //    val is_lr = io.core.commit.uops(w).mem_cmd === rocket.M_XLR &&
  //                !(io.core.commit.uops(w).inst(26) & io.core.commit.uops(w).inst(25))


  //    when (commit_load && is_lr) {
  //      printf("YH+ [%d] Set is_reserved\n", io.core.tsc_reg)
  //      is_reserved := true.B
  //    }

  //    //temp_is_reserved = Mux(commit_load && is_lr, true.B,
  //    //                    Mux(clear_store && is_sc, false.B, temp_is_reserved))
  ...
  ////is_reserved := temp_is_reserved
  printf("YH+ [%d] is_reserved: %d\n", io.core.tsc_reg, is_reserved)
  //yh+end
