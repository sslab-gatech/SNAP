...
class BoomFrontendIO(implicit p: Parameters) extends BoomBundle
{
  ...
  //yh+begin
  val startAddr         = UInt(xLen.W)
  val endAddr           = UInt(xLen.W)
  val enableBRQ         = Bool()
  val enableBrDest      = Bool()
  //yh+end
}
  ...
  with HasL1ICacheBankedParameters
{
  ...
  //yh+begin
  val s0_brDest = Wire(Bool())
  val s1_brDest = Reg(Bool())
  val s2_brDest = RegInit(false.B)

  fetch_controller.io.startAddr := io.cpu.startAddr
  fetch_controller.io.endAddr := io.cpu.endAddr
  fetch_controller.io.enableBRQ := io.cpu.enableBRQ
  fetch_controller.io.enableBrDest := io.cpu.enableBrDest
  //yh+end


  s1_pc := s0_pc
  s1_brDest := s0_brDest //yh+
  ...
  s0_brDest := fetch_controller.io.imem_req.valid && fetch_controller.io.imem_req.bits.brDest //yh+
  fetch_controller.io.imem_resp.bits.brDest := s2_brDest //yh+
