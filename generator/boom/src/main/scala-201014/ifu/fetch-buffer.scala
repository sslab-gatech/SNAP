  ...
  with HasL1ICacheBankedParameters
{
  ...
  for (i <- 0 until fetchWidth) {
    ...
    //yh+begin
    in_uops(i).uses_brq       := io.enq.valid && io.enableBRQ && (io.enq.bits.brDest || !io.enableBrDest) &&
    //in_uops(i).uses_brq       := io.enq.valid && 
				                          io.startAddr <= io.enq.bits.pc &&
                                  io.endAddr > io.enq.bits.pc
    //in_uops(i).uses_brq       := io.enq.valid && io.enq.bits.brDest
    in_uops(i).inst_hash      := Mux(io.enq.valid && io.enableBRQ && (io.enq.bits.brDest || !io.enableBrDest),
    //in_uops(i).hash           := Mux(io.enq.valid,
