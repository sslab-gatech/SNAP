  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (i <- 0 until numBrqEntries)
  {
    ...
    when (reset.asBool || initBRQ.asBool) {
      ...
      //yh-} .elsewhen (i.U === brq_head && brq(i).bits.state === s_done) {
  ...
  // Clear TODO
  //yh-val clear_br = brq(brq_head).valid && brq(brq_head).bits.state === s_done && !io.core.exception
  val clear_br = brq(brq_head).valid && brq(brq_head).bits.state === s_done && !io.core.exception &&
                    brq(brq_head).bits.committed
  ...
  {
    val exe_lb_val = io.core.brinfo.ht_valid && i.U === io.core.brinfo.brq_idx
  ...
  printf("YH+ [%d] stq_head: %d stq_commit_head: %d is_fence: %d ordered: %d\n",
          io.core.tsc_reg, stq_head, stq_commit_head, stq(stq_head).bits.uop.is_fence, io.dmem.ordered);
    ...
    when (io.dmem.resp(w).valid)
    {
      ...
      when (io.dmem.resp(w).bits.uop.is_br_or_jmp)
      {
        when (io.dmem.resp(w).bits.uop.is_fuzz_ld)
        {
          ...
          //yh-brq(io.dmem.resp(w).bits.uop.brq_idx).bits.state := s_store
          ...
          .elsewhen (io.dmem.resp(w).bits.uop.is_fuzz_st)
        {
          ...
          printf("YH+ [%d] Assert resp arrived, brq_idx: %d\n",
                  io.core.tsc_reg, io.dmem.resp(w).bits.uop.brq_idx)
        }
