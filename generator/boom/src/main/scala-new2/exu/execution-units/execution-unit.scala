  ...
  )(implicit p: Parameters) extends BoomBundle
{
  ...
  //val fuzz_io = if (hasBrUnit) Flipped(new boom.lsu.LSUFuzzIO) else null //yh+
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  if (hasAlu) {
    ...
    if (hasBrUnit) {
      ...
      ////yh+begin
      //io.fuzz_io.req.valid := (alu.io.br_unit.brinfo.valid ||
      //                          alu.io.br_unit.brinfo.cfi_type === CFI_JAL)
      //io.fuzz_io.req.bits.uop := alu.io.br_unit.xcpt.bits.uop
      //io.fuzz_io.req.bits.addr := alu.io.br_unit.xcpt.bits.badvaddr
      //io.fuzz_io.mispredict := alu.io.br_unit.brinfo.mispredict
      //io.fuzz_io.taken := alu.io.br_unit.brinfo.taken
      ////yh+end
    }
  }
