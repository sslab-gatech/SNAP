   ...
   with HasBoomCoreIO
{
  ...
  ////yh+
  //exe_units(brunit_idx).io.fuzz_io <> io.lsu.fuzz_exe
...
//      ("DTLB miss",   () => io.dmem.perf.tlbMiss),
  ...
  csr.io.lbqTaken := io.lsu.lbqTaken
  csr.io.lbqMispredict := io.lsu.lbqMispredict
  ...
  for (w <- 0 until coreWidth) {
    ...
    //decode_units(w).io.enq.uop.brq_idx := io.lsu.dis_brq_idx(w) //yh+
  ...
  //dec_fire := (0 until coreWidth).map(w => dec_valids(w) && !dec_stalls(w)
  //              && !(io.ifu.fetchpacket.bits.hash(w).valid && io.lsu.brq_full(w))) //yh+
  ...
  //yh+begin
  //for (w <- 0 until coreWidth) {
  //    when (io.ifu.fetchpacket.valid && dec_fbundle.uops(w).valid &&
  //          !dec_finished_mask(w) && io.ifu.fetchpacket.bits.hash(w).valid && dec_fire(w)) {
  //    }

  //    io.lsu.dis_brq(w).valid := io.ifu.fetchpacket.bits.hash(w).valid && dec_fire(w)

  //    io.lsu.dis_brq(w).uop := dec_uops(w)

  //    io.lsu.dis_brq(w).hash := io.ifu.fetchpacket.bits.hash(w).bits
  //}
  ...
  io.lsu.ht_config.prev := custom_csrs.prev(31,0)
