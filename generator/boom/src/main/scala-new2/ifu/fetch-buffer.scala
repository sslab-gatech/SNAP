...
class FetchBufferResp(implicit p: Parameters) extends BoomBundle
{
  ...
  //val hash = Vec(coreWidth, Valid(Bits(32.W))) //yh+
}
  ...
  with HasL1ICacheBankedParameters
{
  ...
  //val fhb = Reg(Vec(numEntries, Valid(Bits(32.W)))) //yh+
  val deq_vec = Wire(Vec(numRows, Vec(coreWidth, new MicroOp)))
  //val deq_vec_hash = Wire(Vec(numRows, Vec(coreWidth, Valid(Bits(32.W))))) //yh+
  ...
  ////yh+begin
  //val in_hash = Wire(Vec(fetchWidth, Valid(Bits(32.W))))
  ////yh+end
  ...
  for (i <- 0 until fetchWidth) {
    ...
    //yh+begin
    in_uops(i).uses_brq       := io.enq.valid && io.enq.bits.brDest &&
				                          io.startAddr <= io.enq.bits.pc &&
                                  io.endAddr > io.enq.bits.pc
    in_uops(i).hash           := Mux(io.enq.valid && io.enq.bits.brDest,
                                    io.enq.bits.insts(i)(31,16)
                                    ^ io.enq.bits.insts(i)(15,0),
                                    0.U)
    //in_uops(i).uses_brq       := io.enq.valid && io.enq.bits.brDest &&
		//		   (io.startAddr <= io.enq.bits.pc && io.endAddr > io.enq.bits.pc)
    //in_hash(i).valid          := io.enq.valid && io.enq.bits.brDest &&
		//		   (io.startAddr <= io.enq.bits.pc && io.endAddr > io.enq.bits.pc)
    ////in_uops(i).uses_brq       := io.enq.valid && io.enq.bits.brDest
    ////in_hash(i).valid          := io.enq.valid && io.enq.bits.brDest
    //in_hash(i).bits           := Mux(io.enq.bits.brDest,
    //                                io.enq.bits.pc ^ io.enq.bits.insts(i)(31,16)
    //                                ^ io.enq.bits.insts(i)(15,0),
    //                                0.U)
    //yh+end
  }
  ...
  //yh+
  //when (io.enq.bits.brDest && io.enq.valid) {
	//  printf("YH+ Compare result: %d startAddr: %d endAddr: %d enq.bits.pc: %d\n",
  //      (io.startAddr <= io.enq.bits.pc && io.endAddr > io.enq.bits.pc), io.startAddr, io.endAddr, io.enq.bits.pc)
  //}
  ...
  for (i <- 0 until fetchWidth) {
    for (j <- 0 until numEntries) {
      ...
      ////yh+begin
      //when (do_enq && in_mask(i) && enq_idxs(i)(j) && io.enableBRQ) {
      //  fhb(j) := in_hash(i)
      //}
      ////yh+end
    }
  ...
  for (i <- 0 until numEntries) {
    ...
    //deq_vec_hash(i/coreWidth)(i%coreWidth) := fhb(i) //yh+
  }
  ...
  //io.deq.bits.hash zip Mux1H(head, deq_vec_hash) map {case (d,q) => d := q} //yh+
