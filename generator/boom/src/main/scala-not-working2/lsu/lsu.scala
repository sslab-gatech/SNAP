  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val brq_hash_head     = RegInit(0.U(brqAddrSz.W))
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (dis_br_val) {
      ...
      printf("YH+ [%d] Enqueue BRQ Entry, br_enq_idx: %d brq_head: %d brq_tail: %d uopc: %d inst_hash: %x\n",
              io.core.tsc_reg, br_enq_idx, brq_head, brq_tail, io.core.dis_uops(w).bits.uopc,
              io.core.dis_uops(w).bits.inst_hash)
