...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
  ...
  //yh+begin
  val brqPrev = UInt(xLen.W).asInput
  val lbqHead = UInt(xLen.W).asInput
  val lbqAddr = Vec(32, UInt(coreMaxAddrBits.W)).asInput
  val lbqStatus = UInt(xLen.W).asInput
  val baseAddrOfBitmap = UInt(xLen.W).asInput
  val sizeOfBitmap = UInt(xLen.W).asInput
  val fuzzCSR = UInt(xLen.W).asInput
  val startAddr = UInt(xLen.W).asInput
  val endAddr = UInt(xLen.W).asInput
  val brqFwdCnt = UInt(xLen.W).asInput
  val brqCommitCnt = UInt(xLen.W).asInput
  val cacheHitCnt = UInt(xLen.W).asInput
  val cacheMissCnt = UInt(xLen.W).asInput
  //val numCommittedInst = UInt(xLen.W).asInput
  //val numUsesBrqInst = UInt(xLen.W).asInput
  //yh+end
}
...
class CSRFile(
  ...
  val reg_custom = customCSRs.map { csr =>
    //yh-require(csr.mask >= 0 && csr.mask.bitLength <= xLen)
  ...
  //yh+begin
  val regBaseAddrOfBitmap = Reg(UInt(xLen.W))
  val regSizeOfBitmap = Reg(UInt(xLen.W))
  val regFuzzCSR = Reg(UInt(xLen.W))
  val regStartAddr = Reg(UInt(xLen.W))
  val regEndAddr = Reg(UInt(xLen.W))

  val regBrqPrev = Reg(UInt(xLen.W))
  val regLbqAddr = Reg(Vec(32, UInt(coreMaxAddrBits.W)))
  val regLbqStatus = Reg(UInt(xLen.W))
  val regLbqHead = Reg(UInt(xLen.W))

  val regBrqFwdCnt = Reg(UInt(xLen.W))
  val regBrqCommitCnt = Reg(UInt(xLen.W))
  val regBrqHead = Reg(UInt(xLen.W))
  val regBrqTail = Reg(UInt(xLen.W))
  val regBrqCommitHead = Reg(UInt(xLen.W))

  val regCacheHitCnt = Reg(UInt(xLen.W))
  val regCacheMissCnt = Reg(UInt(xLen.W))
  //val regNumCommittedInst = Reg(UInt(xLen.W))
  //val regNumUsesBrqInst = Reg(UInt(xLen.W))

  regBaseAddrOfBitmap := io.baseAddrOfBitmap.asUInt
  regSizeOfBitmap := io.sizeOfBitmap.asUInt
  regFuzzCSR := io.fuzzCSR.asUInt
  regStartAddr := io.startAddr.asUInt
  regEndAddr := io.endAddr.asUInt
  regBrqPrev := io.brqPrev.asUInt
  regBrqFwdCnt := io.brqFwdCnt.asUInt
  regBrqCommitCnt := io.brqCommitCnt.asUInt
  regCacheHitCnt := io.cacheHitCnt.asUInt
  regCacheMissCnt := io.cacheMissCnt.asUInt
  //regNumCommittedInst := io.numCommittedInst.asUInt
  //regNumUsesBrqInst := io.numUsesBrqInst.asUInt

  for (i <- 0 until 32) {
    regLbqAddr(i) := io.lbqAddr(i).asUInt
  }

  regLbqStatus := io.lbqStatus.asUInt
  regLbqHead := io.lbqHead.asUInt
  //regBrqHead := io.brqHead.asUInt
  //regBrqTail := io.brqTail.asUInt
  //regBrqCommitHead := io.brqCommitHead.asUInt

  val baseAddrOfBitmapCSRId = 0x430
  read_mapping += baseAddrOfBitmapCSRId -> regBaseAddrOfBitmap

  val sizeOfBitmapCSRId = 0x431
  read_mapping += sizeOfBitmapCSRId -> regSizeOfBitmap

  val fuzzCSRId = 0x432
  read_mapping += fuzzCSRId -> regFuzzCSR

  val startAddrCSRId = 0x433
  read_mapping += startAddrCSRId -> regStartAddr

  val endAddrCSRId = 0x434
  read_mapping += endAddrCSRId -> regEndAddr

  val brqPrevCSRId = 0x435
  read_mapping += brqPrevCSRId -> regBrqPrev

  val brqFwdCntCSRId = 0x436
  read_mapping += brqFwdCntCSRId -> regBrqFwdCnt

  val brqCommitCntCSRId = 0x437
  read_mapping += brqCommitCntCSRId -> regBrqCommitCnt

  val cacheHitCntCSRId = 0x438
  read_mapping += cacheHitCntCSRId -> regCacheHitCnt

  val cacheMissCntCSRId = 0x439
  read_mapping += cacheMissCntCSRId -> regCacheMissCnt

  //val numCommittedInstCSRId = 0x43a
  //read_mapping += numCommittedInstCSRId -> regNumCommittedInst

  //val numUsesBrqInstCSRId = 0x43b
  //read_mapping += numUsesBrqInstCSRId -> regNumUsesBrqInst

  val lbqAddrCSRId = 0x440
  for (i <- 0 until 32)
    read_mapping += (lbqAddrCSRId + i) -> regLbqAddr(i)

  val lbqStatusCSRId = 0x460
  read_mapping += lbqStatusCSRId -> regLbqStatus

  val lbqHeadCSRId = 0x461
  read_mapping += lbqHeadCSRId -> regLbqHead
  //yh+end
